RSpec.describe ::FFI::Hydrogen do
  ...
  describe "#hydro_secretbox_encrypt" do
    it "encrypts a string which does not match the original" do
      encrypted = ::FFI::Hydrogen.hydro_secretbox_encrypt(str, ctx, key)
      expect(encrypted).not_to eq(str)
    end

    it "encrypts a string differently when context changes" do
      encrypted1 = ::FFI::Hydrogen.hydro_secretbox_encrypt(str, ctx, key)
      encrypted2 = ::FFI::Hydrogen.hydro_secretbox_encrypt(str, "another1", key)
      expect(encrypted1).not_to eq(encrypted2)
    end

    it "encrypts a string differently when message_id changes" do
      encrypted1 = ::FFI::Hydrogen.hydro_secretbox_encrypt(str, ctx, key, 0)
      encrypted2 = ::FFI::Hydrogen.hydro_secretbox_encrypt(str, ctx, key, 1)
      expect(encrypted1).not_to eq(encrypted2)
    end
  end
  ...
  describe "#hydro_secretbox_decrypt" do
    it "decrypts a string which matches the original" do
      encrypted = ::FFI::Hydrogen.hydro_secretbox_encrypt(str, ctx, key)
      decrypted = ::FFI::Hydrogen.hydro_secretbox_decrypt(encrypted, ctx, key)
      expect(decrypted).to eq(str)
    end

    it "encrypts a string differently when context and message_id changes" do
      encrypted = ::FFI::Hydrogen.hydro_secretbox_encrypt(str, "another1", key, 1)
      decrypted = ::FFI::Hydrogen.hydro_secretbox_decrypt(encrypted, "another1", key, 1)
      expect(decrypted).to eq(str)
    end

    it "fails to decrypt when context does not match" do
      encrypted = ::FFI::Hydrogen.hydro_secretbox_encrypt(str, ctx, key)
      decrypted = ::FFI::Hydrogen.hydro_secretbox_decrypt(encrypted, "another1", key)
      expect(decrypted).not_to eq(str)
    end

    it "fails to decrypt when message_id does not match" do
      encrypted = ::FFI::Hydrogen.hydro_secretbox_encrypt(str, ctx, key, 1)
      decrypted = ::FFI::Hydrogen.hydro_secretbox_decrypt(encrypted, ctx, key, 2)
      expect(decrypted).not_to eq(str)
    end
  end

  describe ::FFI::Hydrogen::Secretbox do
